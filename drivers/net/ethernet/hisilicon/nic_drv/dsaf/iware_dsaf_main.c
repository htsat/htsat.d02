 ...
 */
u32 dsaf_get_chip_id(struct platform_device *pdev)
{
...
static int dsaf_irq_init(struct device_node *np, struct dsaf_device *dsaf_dev)
{
	...
	for (idx = 0; idx < DSAF_IRQ_NUM; idx++) {
		irq_num = dsaf_dev->virq[idx] = irq_of_parse_and_map(np, idx);
		if (!dsaf_dev->virq[idx]) {
			log_err(dsaf_dev->dev,
		...
		if (ret) {
			log_err(dsaf_dev->dev,
				"dsaf(%d) request_irq faild! irq=%d, virq=%d, ret=%d\n",
				dsaf_dev->chip_id, irq_num,
				dsaf_dev->virq[idx], ret);
			goto request_irq_fail;
		}
		log_dbg(dsaf_dev->dev,
			"dsaf(%d) request_irq %d virq=%d idx=%d success!\n",
			dsaf_dev->chip_id, irq_num, dsaf_dev->virq[idx], idx);
	}
...
static int __init dsaf_module_init(void)
{
	...
	(void)pr_debug("enter dsaf_module_init!\n");
