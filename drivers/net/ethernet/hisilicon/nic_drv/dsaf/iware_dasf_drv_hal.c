 ...
 */
static void dsaf_init_hw(struct dsaf_device *dsaf_dev)
{
	log_dbg(dsaf_dev->dev,
		"dsaf_init_hw begin dsaf%d !\n", dsaf_dev->chip_id);
	...
	/*init SBM*/
	dsaf_sbm_init((u32)dsaf_dev->chip_id);
	log_dbg(dsaf_dev->dev,
				"dsaf_init_hw dsaf_sbm_init dsaf%d !\n",
				dsaf_dev->chip_id);
...
{
	(void)dsaf_int_handler(dsaf_dev->chip_id, irq_num);
}
