...
/* change apb slave to (sys reg,dbi,context table) */
...
static void hisi_pcie_config_context(struct pcie_port *pp)
{
	...
	for (i = 0x400; i < 0x800; i++)
		hisi_pcie_apb_writel(hisi_pcie, 0x01010101, i * 4);
...
static void hisi_pcie_msi_enable(struct device_node *np, struct hisi_pcie *hisi_pcie)
{
...
MODULE_DEVICE_TABLE(of, phosphor_pcie_of_match);
