 ...
 */
void rcb_irq_enable(struct nic_ring_pair *ring, enum rcb_int_flag flag)
{
 ...
 *@flag:ring flag fro tx or rx
 */
void rcb_irq_disable(struct nic_ring_pair *ring, enum rcb_int_flag flag)
{
	struct rcb_device *rcb_dev = &ring->rcb_dev;
...
static int rcb_common_probe(struct platform_device *pdev)
{
	...
	rcb_common->base_irq = platform_get_irq(pdev, 0);
//	ret = of_property_read_u32(np, "irq-num", (u32*)&rcb_common->base_irq);
	if (!rcb_common->base_irq) {
		ret = -EINVAL;
		log_err(&pdev->dev,
...
int rcb_init(struct platform_device *pdev, enum dsaf_mode dsaf_mode)
{
	...
	new_ring->rcb_dev.port_id_in_dsa = (u8) nic_dev->index;
	...
	pr_info(">> virq[RCB_IRQ_IDX_TX] = %d\n", new_ring->rcb_dev.virq[RCB_IRQ_IDX_TX]);
	pr_info(">> virq[RCB_IRQ_IDX_RX] = %d\n", new_ring->rcb_dev.virq[RCB_IRQ_IDX_RX]);

	if (nic_dev->index >= DSAF_PPE_INODE_BASE)
		new_ring->rcb_dev.port_id_in_dsa = 0;
	...
	for (ring_id = 1; ring_id < ring_pair_num; ring_id++) {
		...
		new_ring->rcb_dev.virq[RCB_IRQ_IDX_TX] = irq_of_parse_and_map(np, 2*ring_id);
		new_ring->rcb_dev.virq[RCB_IRQ_IDX_RX] = irq_of_parse_and_map(np, 2*ring_id + 1);
		pr_info(">> virq[RCB_IRQ_IDX_TX] = %d\n", new_ring->rcb_dev.virq[RCB_IRQ_IDX_TX]);
		pr_info(">> virq[RCB_IRQ_IDX_RX] = %d\n", new_ring->rcb_dev.virq[RCB_IRQ_IDX_RX]);
...
int rcb_irq_init(struct nic_ring_pair *ring)
{
	int ret = 0;
	struct rcb_device *rcb_dev = &ring->rcb_dev;
	s32 irq_num = rcb_dev->virq[RCB_IRQ_IDX_TX];

	log_dbg(ring->dev, "%s tx irq = %d\n", __func__, irq_num);
